...
if grep -qF "$target_str" "$file_path"; then
    # The target string exists, so update it if necessary
    if ! grep -qF "$replacement_str" "$file_path"; then
        # The replacement string does not exist, so replace the target string
        sed -i "s/$target_str/$replacement_str/" "$file_path"
    ...
    echo "Error: ui.display.type value is not set to waveshare_2"
fi
...
if grep -qF "$target_line" "$file_path1"; then
    ...
    echo "Added new line after $target_line"
else
